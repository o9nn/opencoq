...
cyg_install() {
  ...
  # Ensure Cygwin setup directories exist to avoid "No such file or directory" errors
  local CYGWIN_ROOT="${CYGROOT:-/cygdrive/c/cygwin}"
  if [ ! -d "$CYGWIN_ROOT" ]; then
    CYGWIN_ROOT="/cygdrive/c/cygwin64"
  fi
  
  if [ -d "$CYGWIN_ROOT" ]; then
    echo "Ensuring Cygwin setup directories exist in $CYGWIN_ROOT..."
    mkdir -p "$CYGWIN_ROOT/etc/setup"
    # Create minimal setup files if they don't exist
    touch "$CYGWIN_ROOT/etc/setup/setup.rc" 2>/dev/null || true
    touch "$CYGWIN_ROOT/etc/setup/installed.db" 2>/dev/null || true
    touch "$CYGWIN_ROOT/etc/setup/timestamp" 2>/dev/null || true
  fi
  
  # Run installer with retry logic for network failures
  local max_retries=3
  local retry_count=0
  while [ $retry_count -lt $max_retries ]; do
    if cygstart -w "$cygwin" -q -P "$@"; then
      echo "Cygwin installation successful"
      return 0
    else
      retry_count=$((retry_count + 1))
      if [ $retry_count -lt $max_retries ]; then
        echo "Cygwin installation attempt $retry_count failed, retrying..."
        sleep 5
      fi
    fi
  done

  echo "ERROR: Cygwin installation failed after $max_retries attempts"
  return 1
}
...
install_base() {
  ...
  # Pre-create necessary directories for Cygwin compatibility
  mkdir -p "$BASE/etc/setup"
  mkdir -p "$BASE/bin"
  mkdir -p "$BASE/lib"
