...
let attention_competition system node_ids =
  ...
  let winners = List.take (min 5 (List.length sorted)) sorted in
  winners
...
let ecan_cycle system =
  ...
  if List.length system.event_history > 1000 then
    system.event_history <- List.take 1000 system.event_history
...
let get_most_important_atoms system count =
  ...
  List.map fst (List.take (min count (List.length sorted)) sorted)
...
let ecan_system_to_scheme system =
  let events_str = String.concat " " (List.map attention_event_to_scheme (List.take 10 system.event_history)) in
