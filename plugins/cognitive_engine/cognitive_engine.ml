...
let single_cognitive_cycle engine =
  ...
  (* 6. Autonomous goal generation (new!) *)
  if engine.cycle_count mod 10 = 0 then ( (* Every 10 cycles *)
    let goal_generated = trigger_autonomous_goal_generation engine in
    if goal_generated then
      Printf.printf "🎯 Autonomous goals updated in cycle %d\n" engine.cycle_count
  );
...
let trigger_autonomous_goal_generation engine =
  Metacognition.trigger_autonomous_goal_generation engine.metacognitive_system

let get_current_autonomous_goals engine =
  engine.metacognitive_system.self_model.current_goals

let evaluate_goal_achievement engine =
  Metacognition.evaluate_goal_progress engine.metacognitive_system
