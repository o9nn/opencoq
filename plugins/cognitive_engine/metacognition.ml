...
let update_performance_metric self_model metric =
  ...
  if List.length self_model.performance_history > 100 then
    self_model.performance_history <- List.take 100 self_model.performance_history
...
let learn_from_experience system =
  ...
  let recent_results = List.take 10 system.introspection_history in
...
let get_improvement_trajectory system =
  let recent_results = List.take 20 system.introspection_history in
