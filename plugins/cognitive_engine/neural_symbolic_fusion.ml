...
(** Compute attention gradients from neural embedding performance *)
let compute_attention_gradients_from_neural ctx attention_tensor symbolic_ids neural_ids =
  let num_heads = attention_tensor.Attention_system.attention_heads in
  let temporal_depth = attention_tensor.Attention_system.temporal_depth in
  
  (* Compute performance gradients for each symbolic-neural pair *)
  let total_gradients = Array.make (num_heads * temporal_depth) 0.0 in
  let pair_count = min (List.length symbolic_ids) (List.length neural_ids) in
  
  List.iteri (fun i symbolic_id ->
    if i < List.length neural_ids then (
      let neural_id = List.nth neural_ids i in
      
      (* Compute gradients based on binding strength and neural performance *)
      let gradients = compute_symbolic_gradients ctx symbolic_id neural_id in
      let binding_strength = analyze_binding_quality ctx symbolic_id in
      
      (* Distribute gradients across attention heads *)
      Array.iteri (fun j grad_val ->
        let head_idx = j mod num_heads in
        let temporal_idx = (j / num_heads) mod temporal_depth in
        let flat_idx = head_idx * temporal_depth + temporal_idx in
        
        if flat_idx < Array.length total_gradients then
          total_gradients.(flat_idx) <- total_gradients.(flat_idx) +. 
            (grad_val *. binding_strength /. float_of_int pair_count)
      ) gradients
    )
  ) symbolic_ids;
...
(** Update attention tensor using neural-symbolic fusion gradients *)
let neural_guided_attention_optimization ctx ecan_system attention_tensor gradient_config =
  (* Get high-attention symbolic concepts *)
  let focused_atoms = Attention_system.get_focused_atoms ecan_system in
  let symbolic_ids = List.map fst focused_atoms in
  
  (* Get corresponding neural embeddings *)
  let neural_ids = List.filter_map (fun symbolic_id ->
    symbol_to_neural ctx symbolic_id Embedding_Based
  ) symbolic_ids in
  
  if List.length neural_ids > 0 then (
    (* Compute gradients from neural performance *)
    let neural_gradients = compute_attention_gradients_from_neural ctx attention_tensor symbolic_ids neural_ids in
    
    (* Update attention tensor with neural-derived gradients *)
    let representative_node = if List.length symbolic_ids > 0 then List.hd symbolic_ids else 1 in
    Attention_system.update_attention_gradients attention_tensor representative_node neural_gradients;
    
    (* Apply optimization *)
    Attention_system.apply_gradient_optimization ecan_system attention_tensor gradient_config;
    
    (* Update binding strengths based on attention performance *)
    List.iteri (fun i symbolic_id ->
      if i < List.length neural_ids then (
        let neural_id = List.nth neural_ids i in
        let head_importance = Attention_system.compute_attention_head_importance attention_tensor symbolic_id in
        let avg_importance = Array.fold_left (+.) 0.0 head_importance /. float_of_int (Array.length head_importance) in
        
        (* Update binding strength based on attention importance *)
        try
          let binding = Hashtbl.find ctx.bindings symbolic_id in
          let new_strength = min 1.0 (binding.binding_strength +. 0.01 *. avg_importance) in
          let updated_binding = { binding with 
            binding_strength = new_strength;
            last_updated = get_current_time ();
          } in
...
(** Create attention-guided neural embeddings *)
let create_attention_guided_embeddings ctx attention_tensor symbolic_ids =
  let head_importance_map = Hashtbl.create 16 in
  
  (* Compute attention importance for each symbolic concept *)
  List.iter (fun symbolic_id ->
    let importance = Attention_system.compute_attention_head_importance attention_tensor symbolic_id in
    Hashtbl.add head_importance_map symbolic_id importance
  ) symbolic_ids;
  
  (* Create embeddings weighted by attention importance *)
  List.filter_map (fun symbolic_id ->
    match symbol_to_neural ctx symbolic_id Embedding_Based with
    | Some neural_id -> 
        (try
          let importance = Hashtbl.find head_importance_map symbolic_id in
          let avg_importance = Array.fold_left (+.) 0.0 importance /. float_of_int (Array.length importance) in
          
          (* Scale embedding by attention importance *)
          let scaled_tensor_id = Hypergraph.tensor_scale_op ctx.atomspace neural_id avg_importance in
          Some (symbolic_id, scaled_tensor_id)
        with Not_found -> Some (symbolic_id, neural_id))
    | None -> None
  ) symbolic_ids

(** Attention-driven reinforcement learning integration *)
let attention_driven_reinforcement_learning ctx ecan_system attention_tensor learning_outcomes =
  (* learning_outcomes: list of (symbolic_id, success_score) *)
  List.iter (fun (symbolic_id, success_score) ->
    (* Compute current attention importance *)
    let head_importance = Attention_system.compute_attention_head_importance attention_tensor symbolic_id in
    let current_attention = Array.fold_left (+.) 0.0 head_importance in
    
    (* Create reinforcement gradients based on success *)
    let reinforcement_gradients = Array.make (attention_tensor.attention_heads * attention_tensor.temporal_depth) 0.0 in
    
    for i = 0 to Array.length reinforcement_gradients - 1 do
      let head_idx = i mod attention_tensor.attention_heads in
      let temporal_idx = i / attention_tensor.attention_heads in
      
      (* Positive gradients for successful outcomes, negative for failures *)
      let gradient_magnitude = success_score *. (current_attention +. 0.1) in
      reinforcement_gradients.(i) <- gradient_magnitude *. (0.1 +. Random.float 0.1)
    done;
    
    (* Apply reinforcement gradients *)
    Attention_system.update_attention_gradients attention_tensor symbolic_id reinforcement_gradients;
    
    (* Update binding strength based on learning outcome *)
    try
      let binding = Hashtbl.find ctx.bindings symbolic_id in
      let strength_adjustment = success_score *. 0.05 in
      let new_strength = max 0.0 (min 1.0 (binding.binding_strength +. strength_adjustment)) in
      let updated_binding = { binding with 
        binding_strength = new_strength;
        last_updated = get_current_time ();
      } in
...
(** Comprehensive neural-symbolic attention optimization cycle *)
let neural_symbolic_attention_cycle ctx ecan_system attention_tensor gradient_config learning_outcomes =
  (* Phase 1: Apply reinforcement learning from outcomes *)
  attention_driven_reinforcement_learning ctx ecan_system attention_tensor learning_outcomes;
  
  (* Phase 2: Neural-guided attention optimization *)
  neural_guided_attention_optimization ctx ecan_system attention_tensor gradient_config;
  ...
  let fusion_stats = get_fusion_statistics ctx in
  let attention_stats = Attention_system.get_attention_tensor_stats attention_tensor in
  let ecan_stats = Attention_system.get_attention_statistics ecan_system in
  
  (fusion_stats, attention_stats, ecan_stats)